...
public class OperadoresLogicosLogin {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < usernames.length; i++){ //i empieza en 0, iterará mientras i sea menor a la cantidad de elementos, i incrementerá en 1
            esAutenticado = (usernames[i].equals(u) && passwords[i].equals(p))? true: esAutenticado;//Para comparar strins se utiliza equals
            // y usamos AND para verificar, si username es true Y password es true avanza
            // Usamos un oprador ternario, entonces ? verifica Y : devuelve alguno de los dos valores
        ...
        String mensaje = esAutenticado ? "Bienvenido usuario ".concat(u).concat("!") :
                "Username o contraseña incorrecto!\nLo sentimos, requiere autenticación"; // Operador ternario, pregunta si es
        //autenticado, si se cumple es bienvenido : si no se cumple es lo sentimos
